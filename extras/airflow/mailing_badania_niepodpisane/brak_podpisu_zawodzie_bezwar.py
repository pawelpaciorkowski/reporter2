...
SQL = SQL_POSTGRES = """
    ...
    from (
            ...
            WHERE
		    W.DataRejestracji >= cast(current_timestamp - interval '48h' as date) and 
		    W.Wydrukowane is null 
		    and z.pacjent is not null
		    and b.pakiet = '0' and b.symbol not in ('SURWLAS')
                    and W.Zatwierdzone < (current_timestamp - interval '1 hour')
                    and w.WYSLANEZLECENIE is null
...
class RaportBrakPodpisu:
    ...
    def generuj_bp(self):
        ...
        if len(self.braki) > 0:
            ...
            for row in self.braki:
                ...
                wiersz = """<tr><td class="c">%s<br />%d</td><td class="c">%s</td><td class="c">%s</td><td>%s</td><td class="c">%s<br />%s</td><td>%s</td>"""
                wiersz %= (row['data'], row['numer'] or -999999,
                           kod_kreskowy,
                           row['pp'],
    ...
    def wyslij(self, adresy):
        temat = 'Raport dla %s z wyników niepodpisanych elektronicznie / BEZ WARUNKOW' % self.nazwa
