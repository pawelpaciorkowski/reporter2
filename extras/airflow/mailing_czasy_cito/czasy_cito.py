import sys
import datetime
import weasyprint
from datasources.centrum import Centrum, CentrumPostgres, CentrumManager
from datasources.connections import get_centrum_connection
from outlib import Email
import sentry_sdk


sentry_sdk.init("http://f528eea41b4c4339bf56241de3172a2d@2.0.205.117:9000/12")

LOGO = """"""

SQL = """SELECT
    W.DataRejestracji AS DATA,
    Z.Numer AS NUMER,
    T.Symbol AS TYP,
    B.Symbol AS BADANIE,
    o.symbol as ODDZIAL,
    Z.GodzinaRejestracji AS REJESTRACJA,
    (P.Nazwisko || ' ' || P.Imiona) as PACJENT,
    case when w.godzinarejestracji > w.DYSTRYBUCJA then cast (((w.zatwierdzone - w.godzinarejestracji) * 1440) as decimal(18,0)) else cast (((w.zatwierdzone - w.DYSTRYBUCJA) * 1440) as decimal(18,0))  end AS DOZATW,
    case when w.godzinarejestracji > w.DYSTRYBUCJA then cast (((w.Wydrukowane - w.godzinarejestracji) * 1440) as decimal(18,0)) else cast (((w.Wydrukowane - w.DYSTRYBUCJA) * 1440) as decimal(18,0))  end AS DOWYDR
FROM Wykonania W
    LEFT OUTER JOIN Zlecenia Z ON Z.ID = W.Zlecenie
    left outer join Pacjenci P on Z.Pacjent = P.Id
    LEFT OUTER JOIN TypyZlecen T ON T.ID = Z.TypZlecenia
    LEFT OUTER JOIN Badania B ON B.ID = W.Badanie
    left outer join platnicy pl on pl.id=w.platnik
    left outer join grupyplatnikow gp on gp.id=pl.grupa
    left outer join oddzialy o on o.id = z.oddzial
WHERE
    W.DataRejestracji = 'YESTERDAY' and w.BladWykonania is null and w.anulowane is null and
    not exists (select w1.id from wykonania w1 where w1.DataRejestracji = 'YESTERDAY' and w1.zlecenie=W.zlecenie and w1.badanie=w.badanie and w1.material=w.material and w1.powtorka='1' and w1.id<>w.id ) and
    b.symbol in ('DD-IL', 'D-DIMER', 'MORF', 'PT', 'APTT', 'FIBR', 'GLU', 'KREA', 'NA', 'K', 'CL', 'AMYL', 'CRP-IL', 'CK-MB', 'TROP-I', 'TROP-T', 'RKZ', 'RKZ-PAK', 'TSH', 'ETYL', 'GRUPA', 'PR-ZGOD', 'B-HCG', 'NARKOT', 'PMR', 'MOCZ', 'TROPIHS', 'TROPTHS')
    and W.Zatwierdzone IS NOT NULL and gp.symbol like '%SZPI%' and t.symbol = 'C'
ORDER BY W.DataRejestracji, Z.Numer, B.Symbol; """


SQL_WSZ = """SELECT
    count (w.id) as WSZYSTKIE
FROM Wykonania W
    LEFT OUTER JOIN Zlecenia Z ON Z.ID = W.Zlecenie
    LEFT OUTER JOIN TypyZlecen T ON T.ID = Z.TypZlecenia
    left outer join platnicy p on p.id=w.platnik
    left outer join grupyplatnikow gp on gp.id=p.grupa
WHERE
    W.DataRejestracji = 'YESTERDAY'	and w.BladWykonania is null and w.anulowane is null and
    not exists (select w1.id from wykonania w1 where w1.DataRejestracji = 'YESTERDAY' and w1.zlecenie=W.zlecenie and w1.badanie=w.badanie and w1.material=w.material and w1.powtorka='1' and w1.id<>w.id ) and
    W.Zatwierdzone IS NOT NULL and gp.symbol like '%SZPI%' and t.symbol = 'C'; """


SQL_POSTGRES = """SELECT
    W.DataRejestracji AS DATA,
    Z.Numer AS NUMER,
    T.Symbol AS TYP,
    B.Symbol AS BADANIE,
    o.symbol as ODDZIAL,
    Z.GodzinaRejestracji AS REJESTRACJA,
    (P.Nazwisko || ' ' || P.Imiona) as PACJENT,
    case when w.godzinarejestracji > w.DYSTRYBUCJA then cast (((extract(epoch from w.zatwierdzone) - extract(epoch from w.godzinarejestracji)) / 60) as decimal(18,0)) else cast (((extract(epoch from w.zatwierdzone) -extract(epoch from w.DYSTRYBUCJA)) / 60) as decimal(18,0))  end AS DOZATW,
    case when w.godzinarejestracji > w.DYSTRYBUCJA then cast (((extract(epoch from w.Wydrukowane) - extract(epoch from w.godzinarejestracji)) / 60) as decimal(18,0)) else cast (((extract(epoch from w.Wydrukowane) - extract(epoch from w.DYSTRYBUCJA)) / 60) as decimal(18,0))  end AS DOWYDR
FROM Wykonania W
    LEFT OUTER JOIN Zlecenia Z ON Z.ID = W.Zlecenie
    left outer join Pacjenci P on Z.Pacjent = P.Id
    LEFT OUTER JOIN TypyZlecen T ON T.ID = Z.TypZlecenia
    LEFT OUTER JOIN Badania B ON B.ID = W.Badanie
    left outer join platnicy pl on pl.id=w.platnik
    left outer join grupyplatnikow gp on gp.id=pl.grupa
    left outer join oddzialy o on o.id = z.oddzial
WHERE
    W.DataRejestracji = 'YESTERDAY' and w.BladWykonania is null and w.anulowane is null and
    not exists (select w1.id from wykonania w1 where w1.DataRejestracji = 'YESTERDAY' and w1.zlecenie=W.zlecenie and w1.badanie=w.badanie and w1.material=w.material and w1.powtorka='1' and w1.id<>w.id ) and
    b.symbol in ('DD-IL', 'D-DIMER', 'MORF', 'PT', 'APTT', 'FIBR', 'GLU', 'KREA', 'NA', 'K', 'CL', 'AMYL', 'CRP-IL', 'CK-MB', 'TROP-I', 'TROP-T', 'RKZ', 'RKZ-PAK', 'TSH', 'ETYL', 'GRUPA', 'PR-ZGOD', 'B-HCG', 'NARKOT', 'PMR', 'MOCZ', 'TROPIHS', 'TROPTHS')
    and W.Zatwierdzone IS NOT NULL and gp.symbol like '%SZPI%' and t.symbol = 'C'
ORDER BY W.DataRejestracji, Z.Numer, B.Symbol; """


SQL_WSZ_POSTGRES = """SELECT
    count (w.id) as WSZYSTKIE
FROM Wykonania W
    LEFT OUTER JOIN Zlecenia Z ON Z.ID = W.Zlecenie
    LEFT OUTER JOIN TypyZlecen T ON T.ID = Z.TypZlecenia
    left outer join platnicy p on p.id=w.platnik
    left outer join grupyplatnikow gp on gp.id=p.grupa
WHERE
    W.DataRejestracji = 'YESTERDAY'	and w.BladWykonania is null and w.anulowane is null and
    not exists (select w1.id from wykonania w1 where w1.DataRejestracji = 'YESTERDAY' and w1.zlecenie=W.zlecenie and w1.badanie=w.badanie and w1.material=w.material and w1.powtorka='1' and w1.id<>w.id ) and
    W.Zatwierdzone IS NOT NULL and gp.symbol like '%SZPI%' and t.symbol = 'C'; """


class DBData:
    def __init__(self, name, address, alias):
        self.name = name
        self.address = address
        self.alias = alias
        self.db_source = self.get_db_source()
        self.data1 = None
        self.data2 = None

    def get_db_source(self):
        if ':' in self.alias:
            splited = self.alias.split(':')
            self.alias = splited[1]
            if 'pg' == splited[0]:
                return 'postgres'
        return 'firebird'

    def get_data(self):
        if self.db_source == 'firebird':
            cm = CentrumManager(adres=self.address, alias=self.alias,
                                engine=self.db_source)
            c = cm.get_connection()
            with c.connection() as conn:
                self.data1 = conn.raport_slownikowy(SQL)
                self.data2 = conn.raport_slownikowy(SQL_WSZ)

                # with open('data1', 'w+') as f:
                #     f.write(str(self.data1))
                # with open('data2', 'w+') as f:
                #     f.write(str(self.data2))

        if self.db_source == 'postgres':
            cm = CentrumManager(adres=self.address, alias=self.alias,
                                engine=self.db_source)
            c = cm.get_connection()
            with c.connection() as conn:
                self.data1 = conn.raport_slownikowy(SQL_POSTGRES)
                self.data2 = conn.raport_slownikowy(SQL_WSZ_POSTGRES)


class Table:
    ranges = {
        'below_60': {
            'from': 0,
            'to': 60
        },
        'between_60_and_90': {
            'from': 60,
            'to': 90
        },
        'between_90_and_120': {
            'from': 90,
            'to': 120
        },
        'over_120': {
            'from': 120,
            'to': 99999*999999
        }
    }

    def __init__(self, data_23, data_all):
        self.sum = None
        self.percentage = None
        self.table2_row = {}
        self.report_data = {}
        self.create_table2_data_dict()
        self.row_data = data_23
        self.all_cito = data_all
        self.calculate_sum()
        self.append_all_cito()
        self.calculate_percentage()
        self.report_data['sum'] = self.sum
        self.report_data['percentage'] = self.percentage

    def append_all_cito(self):
        if self.all_cito:
            self.sum['all'] = self.all_cito[0]['wszystkie']

    def calculate_sum(self):
        row_sum = {rng: 0 for rng in self.ranges}
        row_sum['all_23'] = 0

        for row in self.row_data:
            dozatw = row['dozatw']
            for rng in self.ranges:
                rng_dict = self.ranges[rng]
                if rng_dict['from'] < dozatw <= rng_dict['to']:
                    row_sum['all_23'] += 1
                    self.all_to_data_row(rng, row)
                    row_sum[rng] += 1
        self.sum = row_sum

    def create_table2_data_dict(self):
        for rng in self.ranges:
            self.table2_row[rng] = []

    def all_to_data_row(self, category, data):
        self.table2_row[category].append(data)

    def calculate_percentage(self):
        percentage = {}
        if self.sum['all_23'] and 'all' in self.sum:
            for rng in self.ranges:
                row_percentage = self.sum[rng] / self.sum['all_23']
                percentage[rng] = round(row_percentage*100, 2)
            percentage_23 = self.sum['all_23']/self.sum['all']
            percentage['all_23'] = round(percentage_23*100, 2)
            self.percentage = percentage


class Body:
    def __init__(self, data, data2, name, time):
        self.name = name
        self.time = time
        self.data = data
        self.data2 = data2
        self.html = '''
        <body>
        {%LOGO%}
        {%PAGE_INFO%}
        {%TITLE%}
        {%INFO_TABLE%}
        {%TIME_TABLE%}
        {%LIST_TABLE%}
        </body>'''

        self.generate_logo()
        self.generate_title()
        self.generate_info()
        self.generate_infotable()
        self.generate_timetable()
        self.generate_listtable()

    def generate_logo(self):
        logo_html = f"""<img id="head" src="data:image/png;base64,{LOGO}" /> """
        self.html = self.html.replace('{%LOGO%}', logo_html)

    def generate_info(self):
        info = f"""<div class="titlebox_right">Laboratorium Alab {self.name} <br />
        Wygenerowano: {self.time.strftime('%Y-%m-%d %H:%M')}</div>"""
        self.html = self.html.replace('{%PAGE_INFO%}', info)

    def generate_title(self):
        self.html = self.html.replace(
            '{%TITLE%}',
            f"""<h2><div class="informujemy_white">{self.name} raport wykonany {self.time.strftime("%d-%m-%Y")}</div></h2>""")

    def generate_infotable(self):
        tb = InfoTable()
        self.html = self.html.replace('{%INFO_TABLE%}', tb.html)

    def generate_timetable(self):
        tb = TimeTable(self.data)
        self.html = self.html.replace('{%TIME_TABLE%}', tb.html)

    def generate_listtable(self):
        tb = ListTable(self.data2)
        self.html = self.html.replace('{%LIST_TABLE%}', tb.html)


class InfoTable:
    def __init__(self):
        self.html = ''
        self.generate_table()

    def generate_table(self):
        self.html += """<br><table><tbody><tr>
            <th>Raport ze średniego czasu wykonania badań Cito dla 23 podstawowych badań:</th>
        </tr>"""
        wiersz = """<tr>
            <td class="c">MORF, PT, APTT, FIBR, GLU, KREA, JONOGRAM, D-DIMER, AMYL, CRP-IL, CK-MB, TROP, RKZ, TSH, ETYL, GRUPA, PR-ZGOD, B-HCG, NARKOT, PMR, MOCZ, TROPTHS, TROPIHS</td>"""
        self.html += wiersz
        self.html += "</tbody></table>"


class ListTable:
    skip = ['below_60']
    def __init__(self, data):
        self.html = ''
        self.data = data
        self.generate_table()

    def create_title(self, range):
        prefix = 'Badania, których czas wykonania wyniósł '
        title = range.replace('below_', 'ponieżej ')
        title = title.replace('between_', 'pomiędzy ')
        title = title.replace('over_', 'powyżej ')
        title = title.replace('_and_', ' , a ')
        title = prefix + title + ' minut'
        return f'<h3>{title}</h3>'

    def generate_table(self):
        if self.data:
            for j in self.data:
                if j in self.skip:
                    continue
                if len(self.data[j]) > 0:
                    range_data = self.data[j]
                    title = self.create_title(j)
                    self.html += f"""<br>{title}<table><tbody><tr>
                        <th>Data zlecenia</th>
                        <th>Numer</th>
                        <th>Pacjent</th>
                        <th>Oddział</th>
                        <th>Badania</th>
                        <th>Czas do zatwierdzenia</th>
                    </tr>"""
                    for i in range_data:
                        wiersz = """<tr>
                            <td class="c">%s</td>
                            <td class="c">%s</td>
                            <td class="c">%s</td>
                            <td class="c">%s</td>
                            <td class="c">%s</td>
                            <td class="c">%s</td>"""
                        wiersz %= (
                            i['data'],
                            i['numer'],
                            i['pacjent'],
                            i['oddzial'],
                            i['badanie'],
                            i['dozatw'],
                            )
                        self.html += wiersz
                    self.html += "</tbody></table>"
        else:
            self.html = """<div class="informujemy_white">Brak wyników</div>"""


class TimeTable:
    def __init__(self, data):
        self.html = ''
        self.data = data
        self.generate_table()

    def generate_table(self):
        if self.data['sum'] and self.data['percentage']:
            self.html = """<br><table><tbody><tr>
                <th rowspan="2">Wszystkie Cito</th>
                <th rowspan="2">Ilość 23 bad. Cito</th>
                <th rowspan="2">%23 bad. Cito</th>
                <th colspan="2">do 60 min</th>
                <th colspan="2">60 - 90 min</th>
                <th colspan="2">90 - 120 min</th>
                <th colspan="2">pow. 120 min</th>
            </tr>
            <tr>
            <th>ilość</th><th>procent</th>
            <th>ilość</th><th>procent</th>
            <th>ilość</th><th>procent</th>
            <th>ilość</th><th>procent</th>
            </tr>"""
            wiersz = """<tr>
                <td class="c">%s</td>
                <td class="c">%s</td>
                <td class="c">%s</td>
                <td class="c">%s</td>
                <td class="c">%s</td>
                <td class="c">%s</td>
                <td class="c">%s</td>
                <td class="c">%s</td>
                <td class="c">%s</td>
                <td class="c">%s</td>
                <td class="c">%s</td>"""
            wiersz %= (
                self.data['sum']['all'],
                self.data['sum']['all_23'],
                str(self.data['percentage']['all_23'])+'%',
                self.data['sum']['below_60'],
                str(self.data['percentage']['below_60'])+'%',
                self.data['sum']['between_60_and_90'],
                str(self.data['percentage']['between_60_and_90'])+'%',
                self.data['sum']['between_90_and_120'],
                str(self.data['percentage']['between_90_and_120'])+'%',
                self.data['sum']['over_120'],
                str(self.data['percentage']['over_120'])+'%',
                )
            self.html += wiersz
            self.html += "</tbody></table>"
        else:
            self.html = """<div class="informujemy_white">Brak wyników</div>"""


class Report:
    def __init__(self, data, data2, name, alias):
        self.html = None
        self.data = data
        self.data2 = data2
        self.time = datetime.datetime.now()
        self.name = name
        self.alias = alias
        self.filename = "%s_%s_%s.pdf" % (
            "CzasyCito",
            self.name,
            self.time.strftime("%d-%m-%Y_%H%M%S"))
        self.generate_html()
        self.generate_head()
        self.generate_body()

    def save_as_pdf(self):
        with open(f'{self.filename}', 'w+') as f:
            f.write(self.html)
            pdf = weasyprint.HTML(string=self.html)
            pdf.write_pdf(f'{self.filename}')

    def generate_html(self):
        self.html = '<html>{%HEAD%}{%BODY%}</html>'

    def generate_body(self):
        body = Body(self.data, self.data2, self.name, self.time)
        self.html = self.html.replace('{%BODY%}', body.html)

    def generate_head(self):
        head = '''
        <head>
        <style type="text/css">
        html, body {
            font-size: 10pt;
            font-family: 'Arial', helvetica, sans-serif;
            margin: 0; padding: 0;
        }
        @media print {
            @page { size: 210mm 297mm; margin: 10mm 10mm }
        }
        img#head {
            width: 40%
        }
        div.titlebox {
            text-align: center;
            background: #eee;
            border: 1px solid #000;
            padding: 3pt;
        }
        div.titlebox_right {
            width: 55%;
            text-align: right;
            float: right;
        }
        div.informujemy {
            text-align: center;
            background: #eee;
            padding: 3pt;
            font-weight: bold;
            margin-top: 5pt;
            margin-bottom: 5pt;
        }
        div.informujemy_white {
            text-align: center;
            padding: 3pt;
            font-weight: bold;
            margin-top: 5pt;
            margin-bottom: 5pt;
        }

        table {
            width: 100%;
        }

        table, table td, table th {
            border: 1px solid #000;
            border-collapse: collapse;
            font-size: 8pt;
        }

        table th {
            background: #ddd;
            text-align: center;
            font-weight: 700;
            padding: 5px;
        }

        table img {
            width: 3cm !important;
            height: 0.5cm !important;
            margin: 0;
            padding: 0;
        }

        td {
            padding: 5px;
        }

        td.c {
            text-align: center;
        }

        div.niezgodnosc {
            border-bottom: 1px dashed #000;
            margin-bottom: 3pt;
            padding-bottom: 3pt;
        }

        </style></head>'''

        self.html = self.html.replace('{%HEAD%}', head)

    def send(self, send_addresses):
        subject = 'Raport dla %s z Czasów Cito' % self.name
        body = "W załączniku znajduje się wygenerowany raport.\n"
        sender = Email()
        send_addresses = send_addresses.strip()
        try:
            sender.log_save(
                    self.filename.replace('.pdf', '.email'),
                    send_addresses.split(' '), subject, body, [self.filename])
            sender.send(send_addresses.split(' '), subject, body, [self.filename])
        except Exception as e:
            sentry_sdk.capture_exception(e)


if __name__ == '__main__':
    name = sys.argv[1]
    address = sys.argv[2]
    alias = sys.argv[3]
    addresses = sys.argv[4]

    db_data = DBData(name, address, alias)
    db_data.get_data()

    tb1 = Table(db_data.data1, db_data.data2)

    rp = Report(tb1.report_data, tb1.table2_row, name, alias)
    rp.save_as_pdf()
    rp.send(addresses)
