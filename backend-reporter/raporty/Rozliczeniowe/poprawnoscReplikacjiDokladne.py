...
MENU_ENTRY = 'Poprawnosc replikacji - dokładne'

REQUIRE_ROLE = ['C-FIN', 'C-ROZL']
# REQUIRE_ROLE = 'ADMIN'

LAUNCH_DIALOG = Dialog(title=MENU_ENTRY, panel=VBox(
    InfoText(text='Poprawnosc replikacji - dokładne'),
    LabSelector(field='laboratoria', title='Laboratoria', symbole_snr=True),
...
def raport_lab(task_params):
    ...
    tabz = []
    with get_centrum_connection(task_params['target'][:7], fresh=False) as conn:
        cols, rows = conn.raport_z_kolumnami(sqlz, (params['dataod'], params['datado']))
        for row in rows:
            tabz.append({
                'grupa': row[0],
                'ilosc': row[1],
