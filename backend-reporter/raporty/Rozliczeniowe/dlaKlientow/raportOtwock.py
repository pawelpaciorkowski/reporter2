...
laby = [{'lab': 'OTWOCK', 'platnicy': ['OT-SZP']}]
...
def raport_lab(task_params):
    ...
    with get_centrum_connection(task_params['target'], fresh=True) as conn:
        ...
        for row in rows:
            ...
            rows_centrum[key] = {
                ...
                'godzina': row[3],
                'datarej': (row[4].strftime('%Y-%m-%d') if row[4] is not None else None),
                'numerzew': row[5]
            }
    for row_snr in rows_snr:
        key = '%s:%s' % (row_snr['sys'], str(row_snr['sysid']))
        row_centrum = rows_centrum.get(key, {'godzina': 'BRAK DANYCH %s' % key, 'datarej': 'BRAK DANYCH', 'numerzew': 'BRAK DANYCH'})
        wynik.append([
            row_centrum['godzina'],
            row_centrum['datarej'],
            row_snr['kod'],
    ...
    return {
        'type': 'table',
        'header': 'Data wykonania badania;Data rejestracji;Kod zlecenia;Oddział zlecający;Lekarz zlecający;Pesel pacjenta;Nazwisko pacjenta;Imię pacjenta;Data urodzenia pacjenta;Nazwa badania;ICD9;Tryb badania;Ilość;Cena badania;Numer zewnętrzny'.split(
            ';'),
        'data': wynik
    }
