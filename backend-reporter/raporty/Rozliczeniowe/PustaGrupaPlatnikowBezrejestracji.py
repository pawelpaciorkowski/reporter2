...
MENU_ENTRY = 'Płatnik Zablokowany Do Rejestracji'
...
# 	          where
#             W.Datarejestracji >= '2014-01-01' and W.Rozliczone between ? and ?
#             and  P.BEZREJESTRACJI = 1 -- ((W.Anulowane is null	and W.Platne = 1 and p.grupa is null and (w.platnik is not null or (w.platnik is null and w.taryfa is null)) and w.bladwykonania is null) or P.BEZREJESTRACJI = 1)
#
#             and tz.symbol not in ('K', 'KW', 'KZ')
#             group by P.Symbol, P.Nazwa, z.datarejestracji, z.numer, z.kodkreskowy
...
def raport_pusta_grupa_platnikow(task_params):
    ...
    with get_centrum_connection(lab) as conn:
        cols, rows = conn.raport_z_kolumnami(
            SQL, [params['dataod'], params['datado']])
    ...
    if len(wiersze) == 0:
        ...
        return {
            'type': 'table',
            'title': lab,
            'header': 'Płatnik Symbol,Płatnik Nazwa,Data rejestracji,Numer,Kod,Badania'.split(','),
            'data': wiersze
        }
