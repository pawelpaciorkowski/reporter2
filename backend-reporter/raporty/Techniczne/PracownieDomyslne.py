...
LAUNCH_DIALOG = Dialog(title=MENU_ENTRY, panel=VBox(
    InfoText(text="""Pracownie domyślne i czasy wykonania dla poszczególnych badań w wybranych laboratoriach.
             Aktualność danych: nie starsze niż 2 godziny, dla pojedynczego badania bieżące, dla Stępińskiej zawsze dane z wczoraj."""),
    LabSelector(multiselect=True, field='laboratoria', title='Laboratoria', pracownie_domyslne=True, pokaz_nieaktywne=True),
...
def raport_lista_badan(task_params):
    ...
    if params['badanie'] is not None:
        ...
        return get_and_cache('pd2:lista_badan', get_data, CACHE_TIMEOUT)
...
def raport_lab(task_params):
    ...
    def get_data(badanie=None):
        ...
        for row in metody_w_badaniach:
            if row['badanie'] is None:
                raise Exception("Puste badanie w powiązaniach metod", task_params['target'], row)
        for row in badania_aktywne:
            if row['badanie'] is None:
                raise Exception("Puste badanie w badaniach aktywnych", task_params['target'], row)
        res = {
    ...
    if params['badanie'] is not None:
        ...
        return get_and_cache('pd2:lab:%s' % task_params['target'], get_data, CACHE_TIMEOUT)
            ...
            for mwb in stan['metody_w_badaniach']:
                ...
                if symbol not in wszystko_o_badaniach:
                    wszystko_o_badaniach[symbol] = { 'grupa_badan': '---', 'grupa_cennikowa': '---', 'nazwa': symbol,
                                                     'pracownie_w_systemach': {}, 'czasy_w_systemach': {}, 'czynne_w_systemach': {} }
                    badania.append(symbol)
                wszystko_o_badaniach[symbol]['pracownie_w_systemach'][system] = mwb['pracownia']
    ...
    if jedno_badanie:
        if lista_badan is not None:
            ...
            res['results'].append({'type': 'info',
                                   'text': 'Pracownie domyślne badania %s (%s), grupa badań %s, grupa cennikowa %s' % (symbol, nazwa, grupa, grupad)})
            data = []
            ...
            for system, stan in badania_w_systemach.items():
                ...
                wiersz.append(mwb['pracownia'] if mwb is not None else '')
                if ba is not None:
        ...
        if tylko_excel:
            ...
            if task_group.progress == 1:
                ...
                res['results'].append({
                    ...
                    'filename': 'PracownieDomyslne_%s.xlsx' % datetime.datetime.now().strftime('%Y-%m-%d'),
                })
