...
datetime.date
MENU_ENTRY = "Dodatkowa sprzedaż gotówkowa - laboratoria z podziałem multi TEST"
...
def examine_haders(headers, end_hdr):
        hdr = deepcopy(headers)
        hdrs = []
        hdrs.append(hdr[0][end_hdr:])
        hdrs.append(hdr[1])
        hdrs.append(hdr[2])
        for h in hdrs[0][3:]:
            h['colspan'] = len(DATE_RANGES) * len(SEXES) * 2 -2
            
        for r in hdrs:
            for t in r:
                t['colspan'] = t['colspan']+2
        hdrs[0][0]['colspan'] = 2
        hdrs[0][1]['colspan'] = 2
        hdrs[0][2]['colspan'] = 2
        for h in hdrs[2]:
...
def examine_tables(data, headers):
    end_hdr = 19
    headers = examine_haders(headers, end_hdr)
    total_response = []
    template = {
        "type": "table",
        "title": None,
    ...
    for lab_rows in data:
        pprint(lab_rows)
        result = []
        row_data = [d.split(',') for d in lab_rows[end_hdr:]]
        max_examines = len(row_data[0])
        for i in range(max_examines):
            row = []
            for r in row_data:
                try:
                    import json
                    ...
                    dict_val = json.loads(new_val)
                    list_val = list(list(dict_val.items())[0])
                    row.append(list_val[0])
                    row.append(list_val[1])
                except IndexError:
                    val = '-'
                    row.append(val)
                    row.append(val)
            # pprint(row)
            result.append(row)
        current_resp = deepcopy(template)
        current_resp['title'] = lab_rows[1]
        current_resp['data'] = result
        current_resp['headers'] = headers
        total_response.append(current_resp)
...
def dzienna_sprzedaz(task_params):
    ...
    new_response = [{
        ...
        "header": [h[0:19] for h in headers],
        "data": [r[0:19] for r in response],
    }]
    tables = examine_tables(response, headers)
    return new_response + tables
